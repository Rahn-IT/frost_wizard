        ...
        let sipper = sipper(|mut sender| async move {
            ...
            let full_size =
                config
                    .payloads
                    .iter()
                    .fold(0u64, |size, payload| match payload.as_ref() {
                        FilePayload::File { contents, .. } => size + contents.len() as u64,
                        FilePayload::Directory { unpacked_size, .. } => size + unpacked_size,
                    }) as f32;
