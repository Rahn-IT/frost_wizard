...
use clap::Parser;
use frost_wizard::AppManifest;
use serde::Deserialize;
use zip::{
    ZipWriter,
    unstable::{LittleEndianReadExt, LittleEndianWriteExt},
    write::SimpleFileOptions,
};

use crate::post_embed::{append_data, search_for_embedded_data};
...
fn main() {
    ...
    {
        let manifest_len = embedded_reader.read_u64_le().unwrap();
        println!("Found manifest length: {}", manifest_len);

        let mut manifest_bytes = vec![0u8; manifest_len as usize];
        embedded_reader.read_exact(&mut manifest_bytes).unwrap();
        let manifest: AppManifest = postcard::from_bytes(&manifest_bytes).unwrap();
    ...
    match args.command {
        Command::Create {
            ...
            let app_manifest = AppManifest::build().name(name).version(version);
            ...
            append_writer
                ...
                .unwrap();
            append_writer.write_all(&app_manifest_bytes).unwrap();
